...
impl Plugin for AsepritePlugin {
    fn build(&self, app: &mut bevy::prelude::App) {
        app.register_type::<AsepriteAnimationState>()
            .add_asset::<AsepriteImage>()
            .add_asset_loader(AsepriteLoader)
            .add_system(check_aseprite_asset_events.before(AsepriteSystems::UpdateAnim))
            .add_system(check_aseprite_spritesheet_entity.before(AsepriteSystems::UpdateAnim))
...
fn check_aseprite_asset_events(
...
fn check_aseprite_spritesheet_entity(
    mut commands: Commands,
    aseprite_image_assets: Res<Assets<AsepriteImage>>,
    mut existing_aseprites: Query<
        (Entity, &mut Handle<AsepriteImage>),
...
impl AssetLoader for AsepriteLoader {
    fn load<'a>(
        ...
        Box::pin(async move {
            ...
            let mut aseprite_atlas =
                TextureAtlasBuilder::default().format(TextureFormat::Rgba8UnormSrgb);
